...
function Servers({ org }: { org: string }) {
  ...
  const create = useMutation({
    ...
    onSuccess: ({ server }) => {
      ...
      setOpen(false)
      navigate(`/orgs/${org}/servers/${server.permalink}`)
    },
      ...
      {servers.data?.servers.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {servers.data?.servers.map((server) => (
            <Link key={server.id} to={`/orgs/${org}/servers/${server.permalink}`}>
...
function Invitations({ org }: { org: string }) {
  const queryClient = useQueryClient()
  const role = useOrgRole(org)
...
function OrgSettings({ org }: { org: string }) {
  const navigate = useNavigate()
...
export default function OrgHome() {
  ...
  const navigate = useNavigate()
  if (!org) return null
  const tab = window.location.pathname.split(`/orgs/${org}`)[1]?.split("/")[1] || "servers"

  return (
    <div>
      <PageHeader title={org} description="Organization" />
      <Tabs value={tab} onValueChange={(value) => navigate(`/orgs/${org}/${value === "servers" ? "" : value}`)}>
      ...
      <Routes>
        <Route index element={<Servers org={org} />} />
        <Route path="members" element={<Members org={org} />} />
        <Route path="invitations" element={<Invitations org={org} />} />
        <Route path="settings" element={<OrgSettings org={org} />} />
        <Route path="*" element={<Navigate to={`/orgs/${org}`} replace />} />
      </Routes>
    </div>
  )
}
