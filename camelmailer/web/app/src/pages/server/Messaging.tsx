...
function InboundQueue({ api }: { api: Api }) {
...
function StatsView({ api }: { api: Api }) {
...
function Templates({ api }: { api: Api }) {
  const queryClient = useQueryClient()
  const templates = useQuery({ queryKey: ["sapi-templates"], queryFn: api.templates.list })
...
export default function Messaging({ org, server }: { org: string; server: string }) {
  ...
  const subtab = params["*"]?.split("/")[0] || "send"
  ...
  return (
    <div>
      <Tabs
        value={subtab}
        onValueChange={(value) =>
          navigate(`/orgs/${org}/servers/${server}/messaging/${value === "send" ? "" : value}`)
        }
      >
      ...
      <Routes>
        <Route index element={<Send api={api} />} />
        <Route path="messages" element={<Messages api={api} />} />
        <Route path="queue" element={<InboundQueue api={api} />} />
        <Route path="stats" element={<StatsView api={api} />} />
        <Route path="streams" element={<Streams api={api} />} />
        <Route path="templates" element={<Templates api={api} />} />
        <Route
          path="*"
          element={<Navigate to={`/orgs/${org}/servers/${server}/messaging`} replace />}
        />
      </Routes>
    </div>
  )
