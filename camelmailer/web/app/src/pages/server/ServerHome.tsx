...
import { Credentials, Domains, Routes as RoutesTab, Suppressions, Webhooks } from "./ResourceTabs"
...
export default function ServerHome() {
  const { org, server: serverPermalink } = useParams<{ org: string; server: string }>()
  const navigate = useNavigate()
  const serverQuery = useQuery({
    queryKey: ["server", org, serverPermalink],
    queryFn: () => adminApi.servers(org!).get(serverPermalink!),
    enabled: Boolean(org && serverPermalink),
  })
  if (!org || !serverPermalink) return null
  const server = serverQuery.data?.server

  const segments = window.location.pathname.split(`/servers/${serverPermalink}`)[1] ?? ""
  const tab = segments.split("/")[1] || "overview"

  return (
    <div>
      <PageHeader
        title={server ? server.name : serverPermalink}
        description={`${org} / ${serverPermalink}`}
        action={
          server?.suspended ? <Badge variant="destructive">suspended</Badge> : undefined
        }
      />
      <Tabs
        value={tab}
        onValueChange={(value) =>
          navigate(
            `/orgs/${org}/servers/${serverPermalink}${value === "overview" ? "" : `/${value}`}`,
          )
        }
      >
      ...
      <Routes>
        <Route
          index
          element={server ? <Settings org={org} server={server} /> : <p className="text-sm text-muted-foreground">Loading…</p>}
        />
        <Route path="domains" element={<Domains org={org} server={serverPermalink} />} />
        <Route path="credentials" element={<Credentials org={org} server={serverPermalink} />} />
        <Route path="routes" element={<RoutesTab org={org} server={serverPermalink} />} />
        <Route path="webhooks" element={<Webhooks org={org} server={serverPermalink} />} />
        <Route
          path="suppressions"
          element={<Suppressions org={org} server={serverPermalink} />}
        />
        <Route path="messaging/*" element={<Messaging org={org} server={serverPermalink} />} />
        <Route
          path="*"
          element={<Navigate to={`/orgs/${org}/servers/${serverPermalink}`} replace />}
        />
      </Routes>
    </div>
  )
