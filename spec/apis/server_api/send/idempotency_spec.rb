...
RSpec.describe "Server Send API - Idempotency", type: :request do
  ...
  describe "/api/v1/send/message with message_ids" do
    ...
    context "when message_ids are provided" do
      let(:message_id1) { "unique-id-1@example.com" }
      let(:message_id2) { "unique-id-2@example.com" }
      let(:params) do
        default_params.merge(
          message_ids: {
            "test1@example.com" => message_id1,
            "test2@example.com" => message_id2
          }
        )
      end
      ...
      it "creates messages with the provided Message-IDs" do
        ...
        expect(message1.message_id).to eq message_id1
        expect(message2.message_id).to eq message_id2
      end
      ...
      it "includes message_id in response" do
        ...
        parsed_body = JSON.parse(response.body)
        expect(parsed_body["data"]["messages"]["test1@example.com"]["message_id"]).to eq message_id1
        expect(parsed_body["data"]["messages"]["test2@example.com"]["message_id"]).to eq message_id2
      end
      ...
      context "when a duplicate Message-ID is sent" do
        ...
        it "returns the same message ID and token" do
          ...
          first_id1 = first_response["data"]["messages"]["test1@example.com"]["id"]
          first_token1 = first_response["data"]["messages"]["test1@example.com"]["token"]
          ...
          expect(second_response["data"]["messages"]["test1@example.com"]["id"]).to eq first_id1
          expect(second_response["data"]["messages"]["test1@example.com"]["token"]).to eq first_token1
        end
      end
        ...
        before do
          ...
          post "/api/v1/send/message",
               ...
               params: default_params.merge(
                 ...
                 message_ids: { "test1@example.com" => message_id1 }
               ).to_json
        end
